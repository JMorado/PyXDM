...
class XDMCalculator:
    ...
    def _compute_density_properties(self, grid_points: np.ndarray, anisotropic: bool = True) -> dict:
        ...
        if self._closed_shell:
            rho = self.mol.obasis.compute_grid_density_dm(self.dm_full, grid_points)
            tau = 2.0 * self.mol.obasis.compute_grid_kinetic_dm(self.dm_full, grid_points)
            nabla = self.mol.obasis.compute_grid_gradient_dm(self.dm_full, grid_points)
            hessian = self.mol.obasis.compute_grid_hessian_dm(self.dm_full, grid_points)

            laplacian = hessian[:, 0] + hessian[:, 3] + hessian[:, 5]
            nabla_mag2 = np.sum(nabla**2, axis=1)

            d = tau - 0.25 * nabla_mag2 / rho
            Q = (laplacian - 2.0 * d) / 6.0

            b_sigma = compute_b_sigma(rho, Q)

            props = {"rho": rho, "b": b_sigma}

            if anisotropic:
                # Calculate the normalized gradient vectors and add them to the properties dict
                epsilon = 1e-12
                nabla_norm = np.linalg.norm(nabla, axis=1)[:, None]
                props["u"] = nabla / (nabla_norm + epsilon)
        else:
    ...
    def _compute_moment_for_atom(
        ...
        if self._closed_shell:
            if not anisotropic:
                b_capped = np.minimum(density_props["b"], r_i)
                term_iso = r_i**order - (r_i - b_capped) ** order
                integrand_iso = density_props["rho"] * term_iso**2
                lambda_iso = grid.integrate(integrand_iso * weights_i)
                return lambda_iso, None
            else:
                b_capped = np.minimum(density_props["b"], r_i)
                moment_mag = r_i**order - (r_i - b_capped) ** order
                term_vec = moment_mag[:, None] * density_props["u"]
                outer = np.einsum("ni,nj->nij", term_vec, term_vec)
                integrand = density_props["rho"][:, None, None] * outer
                lambda_tensor = np.sum(grid.weights[:, None, None] * weights_i[:, None, None] * integrand, axis=0)
